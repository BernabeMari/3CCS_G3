...
namespace StudentTracker.Controllers
{
    ...
    public class TrackingController : ControllerBase
    {
        ...
        public async Task<IActionResult> StartTracking()
        {
            ...
            // Add school coordinates as the first location in the tracking
            // School coordinates: 14°29'27.2"N 121°03'18.4"E converted to decimal
            var schoolLocation = new LocationTracking
            {
                SessionId = session.SessionId,
                Latitude = 14.4908889m, // 14°29'27.2"N in decimal
                Longitude = 121.055111m, // 121°03'18.4"E in decimal
                Timestamp = DateTime.UtcNow
            };

            _context.LocationTrackings.Add(schoolLocation);
            await _context.SaveChangesAsync();

            // Notify connected parents about the initial school location
            ...
            // Fetch additional student info for the notification
            var studentInfo = await _context.Students
                .Where(s => s.StudentId == currentUserId)
                .Select(s => new { s.Username, s.Fullname, s.ProfilePic })
                .FirstOrDefaultAsync();

            foreach (var parentId in connectedParents)
            {
                // Use Group instead of User to ensure the message is delivered
                await _hubContext.Clients.Group(parentId).SendAsync("ReceiveLocation", 
                    currentUserId, 
                    student.Username, 
                    (double)schoolLocation.Latitude, 
                    (double)schoolLocation.Longitude,
