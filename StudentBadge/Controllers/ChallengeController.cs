...
namespace StudentBadge.Controllers
{
    public class ChallengeController : Controller
    {
        ...
        public async Task<IActionResult> AvailableChallenges()
        {
            ...
            // Since all students take the same challenges, we don't need to filter by year level
            ...
            foreach (var challenge in challenges)
            {
                // Check if the teacher has made this school year visible
        ...
        private async Task<bool> IsSchoolYearVisibleToStudents(string teacherId, string yearLevel)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    {
                        // No record found, default to visible
                        return true;
                    }
                }
            }
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                string query = @"
                    ...
                    GROUP BY c.ChallengeId, c.TeacherId, c.ChallengeName, c.ProgrammingLanguage, 
                             c.Description, c.YearLevel, c.CreatedDate, c.LastUpdatedDate, c.IsActive, c.VisibleFromDate
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    using (SqlDataReader reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            ViewData[$"QuestionCount_{challenge.ChallengeId}"] = reader["QuestionCount"] != DBNull.Value ? Convert.ToInt32(reader["QuestionCount"]) : 0;
        ...
        private async Task<int> CreateChallenge(Challenge challenge)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                string query = @"
                    INSERT INTO Challenges (TeacherId, ChallengeName, ProgrammingLanguage, Description, YearLevel, CreatedDate, IsActive, VisibleFromDate)
                    VALUES (@TeacherId, @ChallengeName, @ProgrammingLanguage, @Description, @YearLevel, @CreatedDate, @IsActive, @VisibleFromDate);
                    SELECT SCOPE_IDENTITY();";
        ...
        private async Task UpdateCompletedChallengesScore(string studentId)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    // Then get points earned from completed challenges
                    string submissionsQuery = @"
                        SELECT cs.PercentageScore, cs.PointsEarned, cs.TotalPoints
                        FROM ChallengeSubmissions cs
                        JOIN Challenges c ON cs.ChallengeId = c.ChallengeId
                        WHERE cs.StudentId = @StudentId
                    ...
                    // Calculate the challenge score: (total score / total items of ALL challenges) * 100 * 0.2
                    decimal challengeScore = 0;
                    if (totalItemsInAllChallenges > 0)
                    {
                        challengeScore = ((decimal)totalPointsEarned / totalItemsInAllChallenges) * 100 * 0.2m;
                        
                        // Ensure the score doesn't exceed 20 (100% * 20%)
        ...
        private async Task<List<Challenge>> GetAvailableChallenges()
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                string query = @"
                    ...
                    WHERE c.IsActive = 1
                    GROUP BY c.ChallengeId, c.TeacherId, c.ChallengeName, c.ProgrammingLanguage, 
                             c.Description, c.YearLevel, c.CreatedDate, c.LastUpdatedDate, c.VisibleFromDate, c.IsActive
                    ...
                    using (SqlDataReader reader = await command.ExecuteReaderAsync())
                    {
                        ...
                        while (await reader.ReadAsync())
                        {
                            challenges.Add(new Challenge
                            {
                                ...
                                CreatedDate = Convert.ToDateTime(reader["CreatedDate"]),
                                LastUpdatedDate = reader["LastUpdatedDate"] != DBNull.Value ? Convert.ToDateTime(reader["LastUpdatedDate"]) : null,
                                VisibleFromDate = columnExists && ReaderHasColumn(reader, "VisibleFromDate") && reader["VisibleFromDate"] != DBNull.Value ? Convert.ToDateTime(reader["VisibleFromDate"]) : null,
                                IsActive = Convert.ToBoolean(reader["IsActive"]),
                                Questions = new List<ChallengeQuestion>()
                            });
                        }
