    ...
    public class ProgrammingTestController : Controller
    {
        ...
        private async Task<int> CalculateTestScore(int testId, string studentId)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                string scoreSql = @"
                    SELECT
                        (SELECT SUM(Points) FROM TestAnswers WHERE SubmissionId = @SubmissionId) as EarnedPoints,
                        (SELECT SUM(Points) FROM ProgrammingQuestions WHERE TestId = @TestId) as TotalPoints";
                
                using (var command = new SqlCommand(scoreSql, connection))
                {
                    command.Parameters.AddWithValue("@SubmissionId", submissionId);
                    command.Parameters.AddWithValue("@TestId", testId);
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            int earnedPoints = reader.IsDBNull(0) ? 0 : reader.GetInt32(0);
        ...
        private async Task<bool> UpdateStudentAchievements(string studentId, string testName)
        {
            try
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    ...
                    // For mastery score calculation: each certification/test gives 20% toward the total score
                    const decimal POINTS_PER_CERTIFICATION = 20.0m;
                    
                    // Count certifications/tests in achievements
                    int certificationCount = 0;
                    string[] newAchievementsEntries = newAchievements.Split('|', StringSplitOptions.RemoveEmptyEntries);
                    
                    foreach (var entry in newAchievementsEntries)
                    {
                    ...
                    // Calculate percentage earned (max 100%)
                    decimal percentageEarned = Math.Min(certificationCount * POINTS_PER_CERTIFICATION, 100.0m);
                    
                    // Apply weight (20% of total score)
                    decimal masteryScore = percentageEarned * 0.2m;
                    ...
                    {
                        command.Parameters.AddWithValue("@Achievements", newAchievements);
                        ...
                        if (rowsAffected > 0)
                        {
                            await UpdateTotalScore(connection, studentId);
                        }
        ...
        private async Task<bool> UpdateTotalScore(SqlConnection connection, string studentId)
        {
            try
            {
                ...
                string getScoresQuery = @"
                    SELECT 
                        AcademicGradesScore, 
                        CompletedChallengesScore, 
                        MasteryScore, 
                        SeminarsWebinarsScore, 
                        ExtracurricularScore
                    FROM StudentDetails
                    WHERE IdNumber = @StudentId";
                
                using (var getCmd = new SqlCommand(getScoresQuery, connection))
                {
                    ...
                    using (var reader = await getCmd.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            ...
                            decimal extraScore = reader["ExtracurricularScore"] != DBNull.Value ? Convert.ToDecimal(reader["ExtracurricularScore"]) : 0;
                            
                            decimal totalScore = academicScore + challengesScore + masteryScore + seminarsScore + extraScore;
                            ...
                            // Update total score and badge color
                            ...
                            using (var updateCmd = new SqlCommand(updateTotalQuery, connection))
                            {
                                updateCmd.Parameters.AddWithValue("@Score", totalScore);
                                updateCmd.Parameters.AddWithValue("@BadgeColor", badgeColor);
