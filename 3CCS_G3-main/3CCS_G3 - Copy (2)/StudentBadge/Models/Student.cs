namespace StudentBadge.Models
{
    public class Student
    {
        ...
        {
            if (CompletedChallenges == null || TotalAvailableChallenges <= 0)
            {
                CompletedChallengesScore = 0;
                return;
            }
            ...
            {
                totalPointsEarned += challenge.PercentageScore;
            }
            
            // Total possible points (assuming each challenge has a max score of 100)
            decimal totalPossiblePoints = TotalAvailableChallenges * 100;
            
            // Calculate category score: (Total Points Earned / Total Possible Points) × 100
            decimal categoryScore = (totalPointsEarned / totalPossiblePoints) * 100;
            
            // Apply weight (20%)
            CompletedChallengesScore = categoryScore * 0.2m;
        }
        
        // Calculate Mastery Score (20% of total)
        private void CalculateMasteryScore()
        {
            // For this implementation, each certification/test gives 20% toward the total score
            const decimal POINTS_PER_CERTIFICATION = 20.0m;
            
            // Start with 0% and add 20% for each certification, up to 100%
            ...
            if (!string.IsNullOrEmpty(Achievements))
            {
                ...
                // Look for certification or test achievements
            ...
            // Calculate percentage: each certification is worth 20%, up to a maximum of 100%
            percentageEarned = Math.Min(certificationCount * POINTS_PER_CERTIFICATION, 100.0m);
            
            // Apply weight (20%)
            MasteryScore = percentageEarned * 0.2m;
        }
